...
function renderImages() {
  ...
  card.innerHTML = `
    ...
    <p>DuckDuckGo's public client-side endpoint does not provide the full image-search feed needed for 150+ results. Ayde will not scrape, proxy, or open DuckDuckGo just to fake this feature.</p>
    <p class="muted">The image grid can be enabled later if you add a permitted search API/backend. GitHub Pages alone cannot provide that feed reliably.</p>
  `;
  resultsList.appendChild(card);
  pagination.hidden = true;
}
...
safeToggle.addEventListener('click', () => {
  // Ayde keeps Safe Search enabled. A client-side site should not provide a
  // control intended to deliberately disable explicit-content filtering.
  safeSearch = true;
  renderSafeToggle();
});
renderSafeToggle();
safeToggle.disabled = true;
safeToggle.title = 'Safe Search is always on in this version of Ayde.';
...
document.getElementById('continue-current').addEventListener('click', () => {
  ...
  fallbackText.textContent = 'Ayde stays inside its own interface. This site cannot display the destination here because the destination controls framing with browser security policies such as X-Frame-Options and CSP.';
});
