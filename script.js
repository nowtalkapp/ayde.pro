...
const updatesReopen = document.getElementById('updates-reopen');

// only keep the stuff that actually needs blocking. adult stuff is allowed.
const BLOCKED_SEARCH_TERMS = ['minor', 'child', 'kid', 'suicide', 'self-harm', 'self harm'];

const tabs = [];
let activeId = null;
let nextId = 1;
let selectedCategory = 'all';
let safeSearch = false; // Session-only. Nothing is persisted. off by default so adult results arent filtered.
let currentSearch = '';
...
async function runSearch(query, category = 'all') {
  ...
  try {
    ...
    // grab any images the instant answer actually gives us
    const images = [];
    if (data.Image) {
      ...
      images.push({
        ...
        url: data.AbstractURL || data.Image,
        image: imgUrl,
        description: data.AbstractText || ''
      });
    }
    // some related topics have icons
    for (const item of data.RelatedTopics || []) {
      if (item.Icon && item.Icon.URL) {
        const iconUrl = item.Icon.URL.startsWith('http') ? item.Icon.URL : `https://duckduckgo.com${item.Icon.URL}`;
        if (item.FirstURL) {
          images.push({
            type: 'image',
      ...
      if (item.Topics) {
        for (const nested of item.Topics) {
          if (nested.Icon && nested.Icon.URL && nested.FirstURL) {
            const iconUrl = nested.Icon.URL.startsWith('http') ? nested.Icon.URL : `https://duckduckgo.com${nested.Icon.URL}`;
            images.push({
              type: 'image',
    ...
    if (category === 'images') {
      if (images.length) {
        resultStatus.textContent = `${images.length} image${images.length === 1 ? '' : 's'} from provider.`;
        renderResults();
      } else {
        showUnsupportedCategory('images');
      }
    } else if (category === 'videos' || category === 'news') {
...
function flattenRelatedTopics(data) {
  const out = [];
  // main abstract if present
  if (data.AbstractURL && (data.AbstractText || data.Heading)) {
    out.push({
      FirstURL: data.AbstractURL,
      Text: data.AbstractText || data.Heading || ''
    });
  }
  // official Results array
  for (const item of data.Results || []) {
    if (item.FirstURL) out.push(item);
  }
  // related topics (including nested)
  for (const item of data.RelatedTopics || []) {
    if (item.Topics) {
  ...
  return out.filter(item => {
    if (!item.FirstURL || seen.has(item.FirstURL)) return false;
    seen.add(item.FirstURL);
    return true;
  });
}
...
function showUnsupportedCategory(category) {
  ...
  resultStatus.textContent = `${label} search`;
  resultsList.replaceChildren();
  const card = document.createElement('div');
  card.className = 'empty-card';
  card.innerHTML = `
    <div class="blocked-mark">📷</div>
    <h2>${label} results can't load inside ayde</h2>
    <p>duckduckgo blocks embedding images, videos, and news pages (x-frame-options). this is a browser security rule, not something ayde can bypass on github pages.</p>
    <p class="muted">try switching to "all" or "sites" for the results that do work. ayde still keeps everything private and never opens new tabs.</p>
  `;
...
function renderImages() {
  ...
  items.forEach(item => {
    ...
    card.addEventListener('click', () => openResult(item.url || item.image, item.title));
    grid.appendChild(card);
  });

  resultsList.appendChild(grid);
  resultStatus.textContent = `${items.length} image${items.length === 1 ? '' : 's'} from provider.`;
}
...
document.addEventListener('keydown', event => {
  ...
  if (event.key === 'Escape' && updatesOverlay && !updatesOverlay.hidden) {
    updatesOverlay.hidden = true;
  }
});

function showUpdates() {
  if (updatesOverlay) updatesOverlay.hidden = false;
}
function hideUpdates() {
  if (updatesOverlay) updatesOverlay.hidden = true;
}
if (closeUpdates) closeUpdates.addEventListener('click', hideUpdates);
if (updatesOverlay) {
  updatesOverlay.addEventListener('click', event => {
    if (event.target === updatesOverlay) hideUpdates();
  });
}
if (updatesReopen) updatesReopen.addEventListener('click', showUpdates);

// Start with exactly one in-memory Ayde tab.
makeTab();
// show update logs on first open (small delay so everything is ready)
setTimeout(showUpdates, 50);
