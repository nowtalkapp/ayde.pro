...
function showUnsupportedCategory(category) {
  ...
  // use the html version + ia params; still often blocked by x-frame-options but this is the best client-only option
  const ia = category === 'images' ? 'images' : category === 'videos' ? 'videos' : 'news';
  const kp = safeSearch ? '1' : '-2';
  const ddgUrl = `https://duckduckgo.com/?q=${encodeURIComponent(currentSearch)}&ia=${ia}&iax=${ia === 'images' || ia === 'videos' ? ia : ''}&kp=${kp}`;
  frame.src = ddgUrl;
  card.appendChild(frame);
  const note = document.createElement('p');
  note.className = 'muted media-note';
  note.textContent = 'trying to show duckduckgo ' + label.toLowerCase() + ' inside ayde. if the frame stays blank, the site is blocking embeds (x-frame-options) — ayde still wont force a new tab.';
...
makeTab();
// show update logs on first open
showUpdates();
